...
class SubmissionService {
  ...
  saveDraft({ authorAccountId, submissionId = null, title, identifier, payload = {} }) {
    ...
    if (submissionId) {
      ...
      const updated = this.dataStore.updateById('submissions', Number(submissionId), {
        ...
        authorNames: payloadWithoutFile.authorNames || existing.authorNames || '',
        affiliationOrContact: payloadWithoutFile.affiliationOrContact || existing.affiliationOrContact || '',
        status: 'draft',
        updatedAt: now,
      });
      if (!this._persistDraftFile({ draftId: updated.id, file })) {
        return {
          ok: false,
          code: 'UPLOAD_FAILED',
          errors: { save: 'Unable to save draft manuscript. Please retry later.' },
        };
      }
    ...
    const draft = this.dataStore.insert('submissions', {
      ...
      authorNames: payloadWithoutFile.authorNames || '',
      affiliationOrContact: payloadWithoutFile.affiliationOrContact || '',
      abstract: payloadWithoutFile.abstract || '',
      keywords: payloadWithoutFile.keywords || '',
      status: 'draft',
      createdAt: now,
      updatedAt: now,
    });
    if (!this._persistDraftFile({ draftId: draft.id, file })) {
      return {
        ok: false,
        code: 'UPLOAD_FAILED',
        errors: { save: 'Unable to save draft manuscript. Please retry later.' },
  ...
  _persistDraftFile({ draftId, file }) {
    if (!file) {
      return true;
    }
    ...
    const extension = String(file.name || '').split('.').pop().toLowerCase();
    const sizeMb = Number(file.sizeMb || 0);
    if (!ALLOWED_EXTENSIONS.includes(extension) || sizeMb > MAX_FILE_MB) {
      return false;
    }

    try {
      const current = this._getCurrentManuscript(draftId);
      const resolvedUpload = this.fileUploadService.uploadManuscript(file);
      ...
      this.dataStore.insert('manuscripts', {
        submissionId: Number(draftId),
        fileName: file.name,
        fileType: extension,
        fileSizeMb: sizeMb,
        storageRef: resolvedUpload.storageRef,
        version: current ? Number(current.version || 1) + 1 : 1,
        isCurrent: true,
        uploadedAt: resolvedUpload.uploadedAt,
      });
      return true;
    } catch (error) {
      return false;
  ...
  listDrafts({ authorAccountId }) {
    const drafts = this.dataStore.list(
      'submissions',
      (row) => row.authorAccountId === authorAccountId && row.status === 'draft'
    );
    return drafts.map((draft) => {
      const manuscript = this._getCurrentManuscript(draft.id);
      return {
        ...draft,
  ...
  listSubmittedByAuthor({ authorAccountId }) {
    ...
    return submissions.map((submission) => {
      const manuscript = this.dataStore.findOne(
        'manuscripts',
        (row) => row.submissionId === submission.id && row.isCurrent
      );
      return {
        id: submission.id,
        title: submission.title,
  ...
  async submitDraft({ authorAccountId, submissionId, file }) {
    ...
    const submission = this.dataStore.updateById('submissions', Number(submissionId), {
      status: 'submitted',
