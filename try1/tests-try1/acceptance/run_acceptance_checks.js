...
async function runSC007() {
  ...
  checks.push(
    check(
      ...
      generated.status === 201 && generated.body.previewHtml,
      generated
    )
  );
  ...
  notificationService.setFailDelivery(true);
  const publishOk = await scheduleController.publish({ body: { scheduleId, confirmed: true } });
  const publicView = scheduleController.showPublicSchedule({ body: { scheduleId } });
  checks.push(
    check(
      'SC007-08',
      'Publish final makes schedule public and non-blocking warning is surfaced',
      publishOk.status === 200 && publishOk.body.notificationWarning && publicView.status === 200,
      { publishOk, publicView }
    )
  );
